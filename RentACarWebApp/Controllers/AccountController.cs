...
namespace RentACarWebApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController()
        {
            ...
            this.UserManager =
                    new ApplicationUserManager(new ArangoDB.AspNet.Identity.UserStore<Korisnik>("ArangoConnection"));
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (user.Password == model.Password)
            {
                ...
                //prepravi ovde da bude prikaz neki kad pogresi sifru
            ...
            //var s = _passwordHasher.VerifyHashedPassword(user.PasswordHash, model.Password);

            //var password = user.Password.GetHashCode();

            //var duplicate = await UserManager.FindByNameAsync(user.Email);
            //await UserManager.DeleteAsync(duplicate);
            //var result = await UserManager.CreateAsync(user, model.Password);
            //var result = await UserManager.FindByNameAsync(user.Email);
            ...
            //var result = await SignInManager.PasswordSignInAsync(signedUser.Email, signedUser.Password, model.RememberMe, shouldLockout: false);
            //var result = await SignInManager.SignInAsync(signedUser, isPersistent: false, rememberBrowser: false);
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModelKorisnik model)
        {
            if (ModelState.IsValid)
            {
                var user = new Korisnik { UserName = model.Email, Email = model.Email };
                ...
                //    .BindVar("emailUsera", model.Email)
                //    .Aql(@"
                //                    FOR item IN Korisnik 
                //                        FILTER item.Email == @emailUsera
                //                        RETURN item
                //                    ")
                //    .ToDocuments();
            ...
            //var updateDocumentResult = arangodb.Document
